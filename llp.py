...
def chooseTermin(output):
    ...
    inputString1 = input("Wähle dein Tutorium (Nummer) oder gib etwas anderes ein um abzubrechen: ")
    ...
    if match:
        ...
        inputString2 = input("Wähle dein Termin (Nummer) oder gib etwas anderes ein um abzubrechen: ")
...
def login(session, user, password):
    ...
    loginUrl = BeautifulSoup(r.text, 'html.parser').find('form', id="options")["action"]
    loginSite = session.post(loginUrl, data = {'Username':user, 'Password':password})
    return not (BeautifulSoup(loginSite.text, 'html.parser').title.string == "Anmelden")


def printTutStats(session):
    r = session.get("https://lernziele.charite.de/zend/tutorien/eigene/anbieterId/1")
    stats = BeautifulSoup(r.text, 'html.parser').find('div', id="content").find_all(string=re.compile('^Teilgenommene.+$'), recursive=False)
    for line in stats:
        print(line)
...
def getTerminObjs(correctTableRows):
    ...
    for tableRow in correctTableRows:
        ...
        links = []
        for date in dates:
            links.append(f'https://lernziele.charite.de{date.find_previous("a", href=True)["href"]}')
...
def book(session, url):
    ...
    tut = session.get(url)
    bookForm = BeautifulSoup(tut.text, 'html.parser').find('form', method="POST")
    if bookForm == None:
...
def main():
    ...
    while retry:
        ...
        if not logedIn:
            retry = input("Falsche Email oder Kennwort. Wiederholen? y/n ").lower() == 'y'
        else:
            retry = False
    print(f'{"Anmeldung erfolgreich" if logedIn else ""}\n')

    r = session.get("https://lernziele.charite.de/zend/tutorien/uebersicht/anbieterId/1?idGruppe=alle")
